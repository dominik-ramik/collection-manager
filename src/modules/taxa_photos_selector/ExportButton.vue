<template>
  <div>
    <v-btn
      ...
      variant="text"
      style="background:#fff; color:#e53935; font-weight:600;"
      @click="onExportClick"
    >
      Export taxa photos
    </v-btn>
...
async function onExportClick() {
  try {
    if (!Array.isArray(appStore.checklistData) || appStore.checklistData.length === 0) {
    ...
    for (const taxEntry of (photographedTaxa.value || [])) {
      ...
      for (const folder of (taxEntry.folders || [])) {
        ...
        for (const f of tFiles) {
          ...
          const specimenLabel = `${sMeta.initials || ''} ${sMeta.number || ''}${sMeta.accletter || ''}`.trim()
          photos.push({
            exportName,
      ...
      if (photos.length > 0) {
        ...
        taxExportMap.set(taxKey, {
          ...
          folderName,
          photos: photos.sort((a, b) => a.exportName.localeCompare(b.exportName)),
        })
      } else {
    ...
    for (const { folderName, photos } of taxExportMap.values()) {
      ...
      const subdir = await exportFolder.getDirectoryHandle(folderName, { create: true })
      for (const ph of photos) {
        const destHandle = await subdir.getFileHandle(ph.exportName, { create: true })
    ...
    const checklist = (appStore.checklistData || []).map(x => x.taxonomy || x)
    ...
    const headers = ['scientific name', ...Array.from({ length: maxPhotos }, (_, i) => `photo${i + 1}`)]
    const rows = []
    for (const tax of checklist) {
      ...
      for (let i = 0; i < maxPhotos; i++) {
        if (i < photos.length) {
          const p = photos[i]
          row[`photo${i + 1}`] = `${folderName}/${p.exportName}|${p.specimenLabel}`
        } else {
          row[`photo${i + 1}`] = ''
        }
      }
    ...
    const exportedTaxaCount = Array.from(taxExportMap.values()).filter(v => v.photos.length > 0).length
    exportDoneText.value = `Exported ${copied} photo${copied === 1 ? '' : 's'} across ${exportedTaxaCount} taxon folder${exportedTaxaCount === 1 ? '' : 's'} to "${exportFolder.name}".`
