...
<script setup>
// filepath: d:\Programy\web\collection-manager\src\modules\symbiota_manager\MatchingRecordsTable.vue
...
const headers = [
  { title: '', value: 'actions', sortable: false }, // pen button first
...
const sortedItems = computed(() => {
  // Default sort by occurrenceID
  return [...props.items].sort((a, b) => String(a.occurrenceID).localeCompare(String(b.occurrenceID)))
})

function isSelected(item) {
  return selectedRows.value.some(row => row.occurrenceID === item.occurrenceID)
}
...
const selectedOccids = computed(() =>
  selectedRows.value.map(row => row.occid)
)
